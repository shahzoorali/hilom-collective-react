    ...
    return (
      <section className="section">
        <div className="container" style={{ maxWidth: 520 }}>
          ...
          <div className="panel">
            <p style={{ marginTop: 0 }}>Sign in to see the events you have a place at.</p>
            <button
              type="button"
              className="btn btn-accent btn-block"
              onClick={() => void login('/account/registrations')}
            >
...
function RegistrationCard({ registration }: { registration: MyRegistration }) {
  ...
  const next = registration.charges
...
function StatusPill({ registration }: { registration: MyRegistration }) {
  ...
  const overdue = registration.charges.some(
    (c) => isOutstanding(c.status) && Date.parse(c.due_at) < Date.now(),
  );
