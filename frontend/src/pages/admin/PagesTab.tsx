...
import {
  ...
  adminDuplicatePage,
  adminListPages,
  adminListTrashedPages,
  adminPermanentlyDeletePage,
  adminTrashPage,
  adminUntrashPage,
  type AdminPage,
} from '../../lib/cms';
...
export default function PagesTab({ adminKey }: { adminKey: string }) {
  ...
  const [trashed, setTrashed] = useState<AdminPage[]>([]);
  ...
  async function reloadTrash() {
    try {
      setTrashed(await adminListTrashedPages(adminKey));
    } catch (e) {
      setError((e as Error).message);
    }
  }
  ...
  async function restore(page: AdminPage) {
    setError(null);
    try {
      await adminUntrashPage(adminKey, page.id);
      await reload();
      await reloadTrash();
    } catch (e) {
      setError((e as Error).message);
  ...
  async function purge(page: AdminPage) {
    ...
    setError(null);
    try {
      await adminPermanentlyDeletePage(adminKey, page.id);
      await reloadTrash();
    } catch (e) {
      setError((e as Error).message);
    }
  ...
  async function duplicate(page: AdminPage) {
    ...
    setBusy(true);
    try {
      const copy = await adminDuplicatePage(adminKey, page.id);
      await reload();
      navigate(`/admin/pages/${copy.id}`);
    } catch (e) {
      setError((e as Error).message);
    } finally {
      setBusy(false);
                        ...
                        <tr key={page.id}>
                          ...
                          <td>
                            {page.status === 'scheduled' ? (
                              <ScheduledBadge at={page.scheduled_at} />
                            ) : (
      ...
      {view === 'trash' && (
        <div className="panel">
          ...
          <p className="small muted">
            Trashed pages are hidden from the site and from the main list, but not gone — restore them
            or permanently delete them here. Built-in pages can’t be trashed at all.
          </p>
                ...
                <tbody>
                  {trashed.map((page) => (
                    <tr key={page.id}>
                      ...
                      <td>
                        <span className="small mono muted">{page.slug === 'home' ? '/' : `/${page.slug}`}</span>
                      </td>
                      <td className="small muted">{page.previous_status ?? 'draft'}</td>
                      <td className="small">
                        {page.deleted_at ? new Date(page.deleted_at).toLocaleString() : '—'}
                      </td>
                      <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                        <button className="btn btn-ghost small" onClick={() => restore(page)}>
                          Restore
                        </button>
                        <button
                          className="btn btn-ghost small"
